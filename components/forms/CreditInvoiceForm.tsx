...
export const CreditInvoiceForm: React.FC<{
  onSuccess: () => void;
  onCancel: () => void;
  ...
  const summary = useMemo(() => {
    const subtotal =
      values.items?.reduce((acc, item) => acc + item.quantity * item.unitPrice, 0) || 0;
    const tax = subtotal * 0.15; // 15% VAT estimate
    const total = subtotal + tax;
    const selectedCust = customers?.find((c) => c.id === values.customerId);
    const wouldExceedLimit =
      (selectedCust?.balance || 0) + total > (selectedCust?.creditLimit || 0);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8 space-y-10">
              <FormSection title="Transaction Header" description="Client and timing metadata.">
                <SelectField
                  ...
                  options={
                    customers?.map((c) => ({
                      label: `${c.name} (Limit: ${fmt(c.creditLimit)})`,
                      value: c.id,
                    })) || []
                  }
                  required
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  {fields.map((field, index) => (
                    <div
                      key={field.id}
                      className="grid grid-cols-12 gap-3 items-end p-4 bg-muted/20 rounded-2xl animate-in slide-in-from-left-2 transition-all"
                    >
                  ...
                  {errors.items && (
                    <p className="text-[10px] font-bold text-rose-500 px-2">
                      Line items required with valid pricing.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="lg:col-span-4 space-y-6">
              <ComputedFieldBlock
                ...
                items={[
                  { label: 'Subtotal', value: fmt(summary.subtotal), status: 'neutral' },
                  { label: 'Estimated Tax (15%)', value: fmt(summary.tax), status: 'neutral' },
                  { label: 'Final Total', value: fmt(summary.total), status: 'neutral' },
                  {
                    label: 'Post-Transaction Bal',
                    value: fmt((summary.customer?.balance || 0) + summary.total),
                    status: summary.wouldExceedLimit ? 'error' : 'success',
                    hint: summary.wouldExceedLimit
                      ? 'Warning: Limit Breached'
                      : 'Within Credit Scope',
                  },
                ]}
              />
              ...
              <div className="bg-card border border-border rounded-2xl p-6 space-y-4 shadow-sm">
                ...
                <div className="space-y-3">
                  <p className="text-[11px] text-muted-foreground leading-relaxed">
                    Posting this invoice will add{' '}
                    <span className="font-bold text-foreground">{fmt(summary.total)}</span> to the{' '}
                    <span className="font-black text-blue-600">0-30 Day</span> aging bucket.
                  </p>
                  {summary.wouldExceedLimit && (
                    <div className="p-3 bg-rose-50 border border-rose-100 rounded-xl flex items-start gap-2">
                      ...
                      <p className="text-[10px] text-rose-800 font-bold leading-tight">
                        CREDIT HOLD: Account limit exceeded. Requires Managerial override pin.
                      </p>
                    </div>
                  )}
                </div>
