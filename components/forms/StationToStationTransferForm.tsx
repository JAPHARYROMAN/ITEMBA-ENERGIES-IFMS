...
interface StationToStationFormValues {
  fromTankId: string;
  toTankId: string;
  quantity: string;
}

export function StationToStationTransferForm({ onSuccess, onCancel }: { onSuccess: () => void; onCancel: () => void }) {
  const { register, handleSubmit, formState: { errors } } = useForm<StationToStationFormValues>();
  const queryClient = useQueryClient();
  const { addToast } = useAppStore();

  const mutation = useMutation({
    mutationFn: async (data: StationToStationFormValues) => {
      return apiFetch('transfers/station-to-station', {
        method: 'POST',
        body: {
  ...
  return (
    <form onSubmit={handleSubmit((v) => mutation.mutate(v))} className="space-y-4 p-4">
      ...
      <div className="space-y-1.5">
        <label className="block text-xs font-black uppercase tracking-wider text-muted-foreground">Destination Tank (Station B)</label>
        <FieldInput {...register('toTankId', { required: 'Required' })} className="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm" placeholder="To Tank UUID" />
      ...
      <div className="space-y-1.5">
        ...
        <FieldInput {...register('quantity', { required: 'Required', min: { value: 0.01, message: 'Must be positive' } })} type="number" step="0.01" className="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm" />
