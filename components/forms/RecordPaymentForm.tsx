...
}> = ({ onSuccess, onCancel, initialCustomerId }) => {
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-5 space-y-10">
              <FormSection title="Payment Detail">
                ...
                <SelectField
                  ...
                  options={['Cash', 'Bank Transfer', 'Cheque', 'Credit Card'].map((m) => ({
                    label: m,
                    value: m,
                  }))}
                  required
              ...
              <div className="p-6 bg-slate-900 text-white rounded-3xl space-y-6 shadow-xl shadow-slate-900/10">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span
                      className={`font-black ${values.amount - totalAllocated > 0 ? 'text-amber-400' : 'text-emerald-400'}`}
                    >
                ...
                <button
                  ...
                  disabled={!selectedCustomerId || !values.amount}
                  className="w-full py-3 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all disabled:opacity-30"
                >
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              {isLoadingInvoices ? (
                <div className="p-12 text-center animate-pulse opacity-40 font-black uppercase">
                  Loading Ledger...
                </div>
                ...
                <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 no-scrollbar">
                  {unpaidInvoices.map((inv) => {
                    ...
                    return (
                      <div
                        key={inv.id}
                        className={`p-5 rounded-2xl border transition-all ${isAllocated ? 'bg-primary/5 border-primary shadow-sm' : 'bg-muted/20 border-border'}`}
                      >
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-xs font-black uppercase">{inv.invoiceNumber}</p>
                              <p className="text-[9px] text-muted-foreground uppercase font-bold">
                                {new Date(inv.date).toLocaleDateString()} • Due in{' '}
                                {Math.ceil(
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-xs font-black">
                              ${inv.balanceRemaining.toLocaleString()}
                            </p>
                          </div>
                        </div>
                        {isAllocated && (
                          <div className="flex items-center gap-4 animate-in slide-in-from-top-2">
                            <div className="flex-1">
                              <input
                                ...
                                aria-label={`Allocation amount for invoice ${inv.invoiceNumber}`}
                                onChange={(e) => {
                                ...
                                className="w-full h-10 bg-background border border-input rounded-xl px-4 text-sm font-black text-right tabular-nums focus:ring-2 focus:ring-primary/40 focus:border-primary outline-none transition-all"
                              />
                            </div>
                            <button
                              type="button"
                              ...
                              className="px-3 py-2 bg-primary/10 text-primary rounded-lg text-[9px] font-black uppercase"
                            >
                              Max
                            </button>
                          </div>
