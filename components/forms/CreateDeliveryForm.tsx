...
export const CreateDeliveryForm: React.FC<{ onSuccess: () => void; onCancel: () => void }> = ({
  onSuccess,
  onCancel,
}) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['del-pending'] });
      queryClient.invalidateQueries({ queryKey: ['deliveries'] });
      addToast(t('forms.saveSuccess', { entity: 'Delivery' }), 'success');
      onSuccess();
    },
    onError: (err: unknown) => addToast(getErrorMessage(err, 'Failed to create delivery'), 'error'),
  });
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-10">
              <FormSection
                title="Supplier & Hardware"
                description="Dispatch source and vehicle identification."
              >
                <SelectField
                  name="supplierId"
              ...
              <FormSection
                ...
                description="Product type and volume quantification."
              >
                <SelectField
                  name="productId"
              ...
              {selectedSupplier ? (
                ...
                >
                  ...
                  <div className="space-y-2 mt-4">
                    <div className="flex justify-between text-[10px]">
                      ...
                      <span
                        className={`font-bold ${selectedSupplier.avgVariance < 0 ? 'text-rose-500' : 'text-emerald-500'}`}
                      >
                  ...
                  {selectedSupplier.rating === 'At Risk' && (
                    <div className="mt-6 flex items-start gap-2 p-3 bg-white/50 border border-rose-100 rounded-xl">
                      ...
                      <p className="text-[10px] text-rose-800 leading-snug font-medium italic">
                        High historical shrinkage reported. Physical dip mandatory before discharge.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="p-12 border-2 border-dashed border-border rounded-3xl flex flex-col items-center justify-center text-center opacity-30">
                  ...
                  <p className="text-[10px] font-bold uppercase">
                    Select supplier to view performance audit
                  </p>
                </div>
              )}
            </div>
