...
const schema = z
  ...
  .superRefine((data, ctx) => {
    ...
    const totalCollected =
      data.collections.cash +
      data.collections.card +
      data.collections.mobileMoney +
      data.collections.voucher;
...
export const CloseShiftForm: React.FC<{ onSuccess: () => void; onCancel: () => void }> = ({
  ...
  const { stationId, station } = useActiveStation();

  // Load active shift for the selected station
  const { data: activeShift, isLoading } = useQuery({
    queryKey: ['active-shift', stationId],
    queryFn: () => shiftRepo.getOpen(stationId),
    enabled: !!stationId,
  });
  ...
  const reconciliation = useMemo(() => {
    ...
    const volume = values.readings.reduce(
      (acc, r) => acc + Math.max(0, (r.closingReading || 0) - r.openingReading),
      0,
    );

    const collected =
      (values.collections?.cash || 0) +
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['active-shift', stationId] });
  ...
  const saveDraft = () => {
    ...
    const ok = setStorageItem(CLOSE_SHIFT_DRAFT_KEY, { ...values, _step: step });
    addToast(
      ok ? 'Draft saved. You can restore it later.' : 'Could not save draft',
      ok ? 'success' : 'error',
    );
  };

  const restoreDraft = () => {
    const draft = getStorageItem<Record<string, unknown>>(CLOSE_SHIFT_DRAFT_KEY);
    if (!draft) return;
    const { _step, ...values } = draft;
    if (values.id === activeShift?.id) {
      methods.reset(values as CloseShiftFormData);
      if (typeof _step === 'number') setStep(_step);
      addToast('Draft restored', 'info');
      removeStorageItem(CLOSE_SHIFT_DRAFT_KEY);
    }
  };

  // Auto-restore draft on mount when activeShift is available
  React.useEffect(() => {
    if (activeShift) {
      restoreDraft();
    }
  ...
  if (isLoading)
    return (
      <div className="h-96 flex items-center justify-center font-black animate-pulse">
        SYNCHRONIZING TERMINAL...
      </div>
    );
  if (!activeShift)
    return (
      <div className="p-12 text-center border-2 border-dashed rounded-3xl opacity-40 font-black uppercase">
    ...
    <FormProvider {...methods}>
      <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="h-full">
        <FormShell
          ...
          actions={
            <PermissionGuard permissions={permissionGroups.shiftsClose}>
                  ...
                  {step < steps.length - 1 ? (
                    ...
                    <>
                      <button
                        ...
                        className="px-5 py-2.5 text-xs font-black uppercase tracking-widest text-primary border border-primary/20 rounded-xl hover:bg-primary/5 transition-all flex items-center gap-2"
                      >
                      ...
                      <button
                        ...
                        className="px-8 py-2.5 bg-rose-600 text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-xl shadow-rose-500/20"
                      >
          ...
          {step === 0 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
              <FormSection
                ...
                description="Enter final nozzle readings to determine total discharge volume."
              >
                          ...
                          <tr key={field.id} className="hover:bg-muted/10 group">
                            ...
                            <td className="px-6 py-4">
                              ...
                              <input
                                ...
                                type="number"
                                step="0.01"
                                aria-label={`Closing reading for ${field.nozzleCode}`}
                                className={`w-32 h-10 bg-background border border-input rounded-xl px-3 text-sm font-black text-right tabular-nums focus:ring-2 focus:ring-primary/40 focus:border-primary outline-none transition-all ${
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter' || e.key === 'ArrowDown') {
                                    ...
                                    const next = document.querySelector(
                                      `input[name="readings.${index + 1}.closingReading"]`,
                                    ) as HTMLInputElement;
                                    if (next) {
                                      next.focus();
                                      next.select();
                            ...
                            <td className="px-6 py-4 text-right">
                              <span
                                className={`text-xs font-black ${vol > 0 ? 'text-primary' : 'opacity-20'}`}
                              >
                                {vol.toFixed(2)} L
                              </span>
                            </td>
          ...
          {step === 1 && (
            <div className="space-y-10 animate-in fade-in slide-in-from-right-4">
              <FormSection
                ...
                description="Breakdown of physical collection by tender type."
              >
                <NumberField
                  ...
                  required
                  placeholder="0.00"
                  hint="Verified cash count from till."
                />
                <NumberField
                  ...
                  required
                  placeholder="0.00"
                  hint="Sum of all settlement receipts."
                />
                <NumberField
                  name="collections.mobileMoney"
                  label="Mobile Money ($)"
                  required
                  placeholder="0.00"
                />
                <NumberField
                  name="collections.voucher"
              ...
              {reconciliation.variance !== 0 && (
                <div className="animate-in slide-in-from-top-4 duration-300">
                  <FormSection
                    ...
                    description="Audit requirement: Explain discrepancies between sales and collections."
                  >
                    <TextareaField
                      name="varianceReason"
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
              <ComputedFieldBlock
                items={[
                  {
                    label: 'Total Volume',
                    value: `${reconciliation.volume.toLocaleString()} L`,
                    status: 'neutral',
                  },
                  {
                    label: 'Expected Revenue',
                    value: fmt(reconciliation.revenue),
                    status: 'neutral',
                  },
                  {
                    label: 'Total Collected',
                    value: fmt(reconciliation.collected),
                    status: 'neutral',
                  },
                  {
                    label: 'Variance',
                    value: fmt(reconciliation.variance),
                    status:
                      reconciliation.variance < 0
                        ? 'error'
                        : reconciliation.variance > 0
                ...
                <div className="flex items-center gap-4 border-b border-border pb-6">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">
                      Compliance metrics within valid thresholds
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="p-4 bg-muted/20 rounded-xl">
                    ...
                    <p className="text-xs font-bold">
                      {station?.code ?? station?.name ?? stationId}
                    </p>
                  </div>
                  <div className="p-4 bg-muted/20 rounded-xl">
                    <p className="text-[9px] font-black uppercase text-muted-foreground mb-1">
                  ...
                  <div className="p-4 bg-muted/20 rounded-xl">
                    ...
                    <span className="px-2 py-0.5 bg-rose-500 text-white text-[8px] font-black uppercase rounded">
                      Closing
                    </span>
                  </div>
                </div>
          ...
          <div className="bg-white text-slate-900 w-full max-w-sm rounded-[2rem] p-8 space-y-6 shadow-2xl relative overflow-hidden font-mono border-t-[12px] border-primary">
            ...
            <div className="border-y border-dashed border-slate-300 py-4 space-y-2 text-[11px]">
              <div className="flex justify-between">
                ...
                <span className="font-black">
                  {(station?.name ?? stationId ?? '').toUpperCase()}
                </span>
              </div>
              <div className="flex justify-between">
                <span>OPERATOR:</span>{' '}
                <span className="font-black">{(user?.name ?? 'UNKNOWN').toUpperCase()}</span>
              </div>
              <div className="flex justify-between">
                <span>TIME:</span> <span className="font-black">{new Date().toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-black">{reconciliation.volume.toFixed(2)} L</span>
              </div>
              <div className="flex justify-between text-xs">
                <span>TOTAL REV:</span>{' '}
                <span className="font-black">{fmt(reconciliation.revenue)}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span>TOTAL COLL:</span>{' '}
                <span className="font-black">{fmt(reconciliation.collected)}</span>
              </div>
              <div className="h-px bg-slate-200" />
              <div className="flex justify-between text-sm font-black">
                <span>VARIANCE:</span>
                <span
                  className={reconciliation.variance < 0 ? 'text-rose-600' : 'text-emerald-600'}
                >
