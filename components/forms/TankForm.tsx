...
export const TankForm: React.FC<TankFormProps> = ({ onSuccess, onCancel, initialData }) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      addToast(t('forms.saveSuccess', { entity: 'Tank' }), 'success');
      onSuccess();
    },
    onError: (err: unknown) => addToast(getErrorMessage(err, 'Failed to create tank'), 'error'),
  });
  ...
  const summary = useMemo(() => {
    ...
    const hasWarnings =
      Object.keys(errors).length > 0 || (values.minLevel || 0) > (values.maxLevel || 0);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="h-full flex">
        <div className="flex-1 overflow-hidden">
            ...
            actions={
              <PermissionGuard permissions={permissionGroups.setupWrite}>
                <button
                  ...
                  onClick={() =>
                    isDirty
                      ? window.confirm('Unsaved changes will be lost. Continue?') && onCancel()
                      : onCancel()
                  }
          ...
          >
            <FormSection
              title="Identity"
              description="Unique identifiers and administrative codes."
            >
              <TextField
                name="code"
                label="Tank Reference Code"
                required
                placeholder="e.g. TNK-SOUTH-01"
              />
              <SelectField
                name="productId"
            ...
            <FormSection
              ...
              description="Physical capacity and automated alarm thresholds."
            >
              ...
              <NumberField
                name="minLevel"
                label="Low Alarm Threshold (L)"
                required
                hint="System triggers low stock warning."
              />
              <NumberField
                name="maxLevel"
                label="Safe Fill Threshold (L)"
                required
                hint="Recommended max fill to prevent spill."
              />
            </FormSection>

            <FormSection
              title="Notes & Attachments"
              description="Audit trails and physical verification notes."
            >
              <TextareaField
                name="notes"
                label="Administrative Notes"
                fullWidth
                placeholder="Maintenance history, physical dip notes..."
              />
            </FormSection>

            {initialData && (
              <FormFooterMeta updatedAt={new Date().toISOString()} updatedBy="Alex Manager" />
            )}
          </FormShell>
        </div>
        ...
        <div className="hidden lg:flex w-72 border-l border-border bg-muted/10 p-6 flex-col gap-8 animate-in slide-in-from-right-4 duration-500">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="text-sm font-black text-primary">
                  {summary.safeVolume.toLocaleString()} L
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="pt-6 border-t border-border/50">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <div
                  className={`p-1 rounded-full ${summary.hasWarnings ? 'bg-rose-500/10 text-rose-500' : 'bg-emerald-500/10 text-emerald-500'}`}
                >
              ...
              <div className="flex items-center gap-3">
                <div
                  className={`p-1 rounded-full ${!values.stationId ? 'bg-amber-500/10 text-amber-500' : 'bg-emerald-500/10 text-emerald-500'}`}
                >
          ...
          <div className="mt-auto p-4 bg-primary/5 rounded-2xl border border-primary/10">
            ...
            <p className="text-[10px] text-muted-foreground leading-tight">
              Setting the Safe Fill Threshold to 95% of capacity is recommended by international
              safety standards.
            </p>
          </div>
        </div>
