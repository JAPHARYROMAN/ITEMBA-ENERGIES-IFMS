...
export const ExpenseEntryForm: React.FC<{
  ...
  const canWriteExpenses = hasAnyPermission(user, permissionGroups.expensesWrite);
  const isExisting = !!initialData;
  const isFormLocked = isExisting || !canWriteExpenses;
  ...
  const mutation = useMutation({
    mutationFn: (data: ExpenseFormData) => {
      if (isExisting && initialData?.id) {
        return expenseRepo.update(initialData.id, data);
      }
    ...
    onSuccess: (saved: any) => {
      ...
      if (!isExisting && (approvalId || approvalStatus === 'submitted')) {
        addToast(
          approvalId
            ? `Approval required. Request ${approvalId} created.`
            : 'Approval required. Request created.',
          'info',
    ...
    onError: (err: any) => {
      ...
      const approvalRequestId =
        err?.apiError?.approvalRequestId ?? nested?.approvalRequestId ?? details?.approvalRequestId;
      if (approvalRequestId) {
        addToast(`Approval required. Request ${approvalRequestId} created.`, 'info', {
          label: 'Open Approval',
          ...
          actions={
            <div className="flex justify-between items-center w-full">
              ...
              <div className="flex gap-3">
                {isExisting && canWriteExpenses && initialData.status === 'Submitted' && (
                  <>
                    <button
                      type="button"
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        approvalMutation.mutate({ id: initialData.id, status: 'Approved' })
                      }
                ...
                {!isExisting && canWriteExpenses && (
                  <button
                    type="submit"
                    className="px-8 py-2.5 bg-primary text-primary-foreground rounded-xl text-xs font-black uppercase tracking-widest shadow-xl shadow-primary/20"
                  >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-8 space-y-10">
              ...
              <FormSection
                ...
                description="Transaction location and categorization."
              >
                <SelectField
                  name="branchId"
              ...
              <FormSection
                ...
                description="Vendor specifics and internal tagging."
              >
                <TextField
                  name="vendor"
                  label="Vendor Name"
                  required
                  placeholder="e.g. Total Energy Tech"
                  disabled={isFormLocked}
                />
                <SelectField
                ...
                <TextareaField
                  ...
                  fullWidth
                  placeholder="Elaborate on the need for this discharge..."
                  disabled={isFormLocked}
                />
              </FormSection>
              ...
              <div className="bg-muted/30 border border-border rounded-2xl p-6 space-y-4 shadow-sm">
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    <div
                      className={`mt-1 p-1 rounded-full ${watchAmount > POLICY_THRESHOLD ? 'bg-amber-500' : 'bg-emerald-500'}`}
                    ></div>
                    <div className="text-[11px] leading-relaxed">
                      <p className="font-bold text-foreground">Threshold Review</p>
                      <p className="text-muted-foreground">
                        Amounts over{' '}
                        <span className="font-black text-foreground">${POLICY_THRESHOLD}</span>{' '}
                        mandate manager multi-factor authorization.
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="text-[11px] leading-relaxed">
                      ...
                      <p className="text-muted-foreground">
                        Ensure VRN/TIN is visible on uploaded evidence for VAT reclaim eligibility.
                      </p>
                    </div>
                  </div>
