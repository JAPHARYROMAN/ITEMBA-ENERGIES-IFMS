...
const schema = z
  ...
  .refine((d) => d.pumpId || (d.pumpCode && d.pumpCode.length > 0), {
...
export const NozzleSetupForm: React.FC<{
  ...
  const mutation = useMutation({
    ...
    onError: (err: unknown) => addToast(getErrorMessage(err, 'Failed to create nozzle'), 'error'),
  });
    ...
    <FormProvider {...methods}>
      <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="h-full">
        ...
        >
          <FormSection
            title="Identity"
            description="Hardware identifiers for the pump and nozzle point."
          >
            {pumps && pumps.length > 0 ? (
              <SelectField
                name="pumpId"
                label="Pump"
                options={pumps.map((p) => ({ label: p.code, value: p.id }))}
                required
              />
            ) : (
            ...
            <TextField
              ...
              required
              placeholder="e.g. N-01-A"
            />
          </FormSection>

          <FormSection
            title="Mapping & Logic"
            description="Financial and physical inventory assignment."
          >
            <SelectField
              name="stationId"
            ...
            <SelectField
              ...
              options={
                tanks?.map((t) => ({ label: `${t.code} (${t.productId})`, value: t.id })) || []
              }
              required
          ...
          <FormSection
            ...
            description="Enable or disable this sales point in the system."
          >
            <SelectField
              name="status"
          ...
          {initialData && (
            <FormFooterMeta updatedAt={new Date().toISOString()} updatedBy="System Auditor" />
          )}
        </FormShell>
      </form>
    </FormProvider>
