...
export const CustomerManagement: React.FC<{
  ...
  const canManageCredit = hasAnyPermission(user, permissionGroups.creditWrite);
  const [activeTab, setActiveTab] = useState('profile');
  const [showInvoiceDrawer, setShowInvoiceDrawer] = useState(false);
  const [showPaymentDrawer, setShowPaymentDrawer] = useState(false);
  const [editingNoteId, setEditingNoteId] = useState<number | null>(null);
  const [noteContent, setNoteContent] = useState(
    'Customer requested extension for Invoice INV-1002 due to regional liquidity crunch. Approved by Alex Manager for 15 days extra.',
  );
  ...
  if (initialId && isLoadingCustomer)
    return (
      <div className="p-12 text-center animate-pulse font-black uppercase">
        Retrieving Master Record...
      </div>
    );
    ...
    <div className="flex flex-col lg:flex-row h-full">
          ...
          >
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className="text-lg font-black tracking-tight">
                  {initialId ? 'Update Identity' : 'Register Customer'}
                </h3>
            ...
            <PermissionGuard permissions={permissionGroups.creditWrite}>
              <div className="flex gap-3 pt-6 border-t border-border">
                <button
                  type="submit"
                  className="flex-1 py-3 bg-primary text-primary-foreground rounded-xl text-xs font-black uppercase tracking-widest shadow-xl shadow-primary/20"
                >
                ...
                <button
                  type="button"
                  onClick={onCancel}
                  className="px-5 py-3 text-[10px] font-black uppercase text-muted-foreground hover:bg-muted rounded-xl transition-all"
                >
                  Cancel
                </button>
              </div>
        ...
        {!initialId ? (
          <div className="flex-1 flex flex-col items-center justify-center opacity-30 text-center p-12">
            ...
            <p className="max-w-xs text-sm font-bold">
              Save the master profile to enable ledger views and transaction history.
            </p>
          </div>
        ) : (
            ...
            <div className="px-8 pt-8 bg-card border-b border-border">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="bg-muted px-4 py-2 rounded-xl border border-border">
                    ...
                    <p className="text-lg font-black">
                      {(((customer?.balance || 0) / (customer?.creditLimit || 1)) * 100).toFixed(1)}
                      %
                    </p>
                  </div>
                </div>
                <div className="flex gap-2">
                  {canManageCredit && (
                    <button
                      ...
                      className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-opacity"
                    >
                      New Invoice
                    </button>
                  )}
                  {canManageCredit && (
                    <button
                      ...
                      className="px-4 py-2 bg-emerald-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-opacity"
                    >
                      Post Payment
                    </button>
                  )}
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="bg-transparent border-b-0 gap-8 h-auto p-0">
                  <TabsTrigger
                    value="profile"
                    className="data-[state=active]:border-primary border-b-2 border-transparent rounded-none px-1 pb-4 h-auto bg-transparent shadow-none font-black uppercase text-[10px] tracking-widest text-muted-foreground data-[state=active]:text-primary"
                  >
              ...
              <Tabs value={activeTab} className="h-full">
                <TabsContent value="profile" className="mt-0 h-full">
                  <div className="bg-card border border-border rounded-3xl p-8 shadow-sm">
                    <h4 className="text-sm font-black uppercase tracking-[0.2em] text-primary mb-6">
                    ...
                    <div className="space-y-4">
                      ...
                      {[1, 2, 3].map((i) => (
                        <div
                          key={i}
                          className="flex items-center justify-between p-4 bg-muted/20 border border-border rounded-2xl hover:bg-muted/40 transition-all cursor-pointer"
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-muted-foreground">
                                Generated on 31 Oct • Cycle Completed
                              </p>
                            </div>
                          </div>
                          <button
                  ...
                  <div className="space-y-6">
                        ...
                        <>
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-[9px] font-black uppercase text-muted-foreground/60">
                              24 Oct 2024 • Admin Audit
                            </span>
                            {canManageCredit && (
                              <button
                                type="button"
                                onClick={() => setEditingNoteId(0)}
                    ...
                    {showNewNote ? (
                      <div className="p-6 border border-border rounded-2xl bg-card space-y-4">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="px-4 py-2 text-[10px] font-black uppercase text-muted-foreground hover:bg-muted rounded-xl"
                          >
                            Cancel
                          </button>
                          <button
                      ...
                      <button
                        ...
                        className="w-full py-4 border border-dashed border-border rounded-2xl text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:bg-muted/30 transition-all"
                      >
      ...
      >
        <CreditInvoiceForm
          ...
          onCancel={() => setShowInvoiceDrawer(false)}
        />
      </DetailsDrawer>
      <DetailsDrawer
      ...
      >
        <RecordPaymentForm
          ...
          onCancel={() => setShowPaymentDrawer(false)}
        />
      </DetailsDrawer>
    </div>
  );
