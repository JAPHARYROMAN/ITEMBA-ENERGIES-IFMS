...
export const ExpenseSummaryDrawer: React.FC = () => {
  ...
  if (isLoading)
    return (
      <div className="p-12 text-center animate-pulse opacity-40 font-black">
        {t('common.loading')}
      </div>
    );
  ...
  return (
    <div className="p-8 space-y-10">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-black tracking-tight">{t('expenseSummary.title')}</h3>
          <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest mt-1">
            Current Billing Cycle • Monthly Aggregation
          </p>
        </div>
        <button
          type="button"
          onClick={() => {
            ...
            downloadCSV(
              `expense-summary-${new Date().toISOString().slice(0, 10)}.csv`,
              headers,
              rows,
            );
          }}
      ...
      <div className="bg-primary/5 border border-primary/10 rounded-2xl p-6 flex items-start gap-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed italic">
            "Maintenance costs for this cycle have increased by 14% compared to the 90-day baseline,
            primarily due to equipment upgrades in Lubricant Bay."
          </p>
        </div>
      </div>
    </div>
