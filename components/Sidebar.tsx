    ...
    <div className="mb-0.5">
      {hasChildren ? (
          ...
          >
            <div className="flex items-center gap-3">
              <div className="relative">
                <item.icon
                  size={16}
                  className={`${isActive ? (isReports ? 'text-indigo-600' : 'text-primary') : 'group-hover:text-foreground opacity-70'}`}
                />
              ...
              {!collapsed && (
                <span
                  className={`font-bold text-xs tracking-tight ${isReports ? 'text-indigo-900 dark:text-indigo-200' : ''}`}
                >
                  {item.name}
                </span>
              )}
            ...
            <div className="ml-4.5 mt-1 flex flex-col gap-0.5 border-l-2 border-border pl-4 py-1 animate-in slide-in-from-left-2 duration-200">
              {item.children?.map((child) => (
                <NavLink
                  ...
                  to={child.path}
                  className={({ isActive }) =>
                    `px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all relative ${
                      isActive
                        ? isReports
                ...
                >
                  {isActive && (
                    <div
                      className={`absolute left-[-18px] top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full ${isReports ? 'bg-indigo-600' : 'bg-primary'}`}
                    />
                  )}
                  {child.name}
                </NavLink>
        ...
        >
          <item.icon
            size={16}
            className={`${isActive ? 'text-primary' : 'group-hover:text-foreground opacity-70'}`}
          />
...
const Sidebar: React.FC = () => {
  ...
  const filterItem = React.useCallback(
    (item: SidebarItem): SidebarItem | null => {
      const visibleChildren = item.children
        ?.map(filterItem)
        .filter((child): child is SidebarItem => child !== null);
      const hasSelfAccess = matchesPermissionRequirement(
        user,
        item.permissions,
  ...
  const filteredNav = NAV_ITEMS.map(filterItem).filter(
  ...
  const categories = ['General', 'Reports', 'Operations', 'Finance', 'Governance', 'Setup'].filter(
    (c) => groupedNav[c],
  );
  ...
  return (
    ...
    >
      <Link
        to="/app/dashboard"
        className="h-16 flex items-center px-5 border-b border-border bg-muted/10 hover:bg-muted/20 transition-colors cursor-pointer"
        ...
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          {!sidebarCollapsed && (
            <div className="flex flex-col animate-in fade-in slide-in-from-left-4">
              ...
              <span className="text-[9px] font-black text-indigo-600 uppercase tracking-widest mt-1 opacity-80">
                Intelligence Hub
              </span>
            </div>
          )}
      ...
      <div className="p-4 border-t border-border bg-muted/5">
        ...
        >
          {sidebarCollapsed ? (
            <ChevronRight size={18} className="group-hover:text-primary transition-all" />
          ) : (
