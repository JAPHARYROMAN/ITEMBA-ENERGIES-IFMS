...
import { payablesAgingRepo } from '../../lib/repositories';
...
export default function PayablesAgingPage() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: payablesAgingRepo.getReport,
  });

  return (
    <div className="space-y-6">
      <PageHeader
        title={t('payables.aging', 'Payables Aging')}
        description={t('payables.agingDesc', 'Accounts payable aging analysis')}
      />
      ...
      {data && (
        <>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <div className="rounded-xl border border-border bg-card p-4">
              <p className="text-xs font-black uppercase tracking-wider text-muted-foreground">Total Invoices</p>
                  ...
                  <div key={bucket.bucket} className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-muted-foreground">
                        {formatCurrency(bucket.amount)} ({bucket.count} invoices)
                      </span>
                    </div>
