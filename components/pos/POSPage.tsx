...
const schema = z
  ...
  .superRefine((data, ctx) => {
    ...
    const totalPaid =
      data.payment.cash + data.payment.card + data.payment.mobile + data.payment.voucher;
...
const POSPage: React.FC = () => {
  ...
  const { data: nozzles } = useQuery({
    ...
    queryFn: () => nozzleRepo.list(stationId),
    enabled: !!stationId,
  });
  ...
  const compatibleNozzles = useMemo(() => {
    if (!nozzles) return [];
    if (!values.productId) return nozzles;
    return nozzles.filter((nozzle) => nozzle.productId === values.productId);
  }, [nozzles, values.productId]);

  useEffect(() => {
    if (values.nozzleId && !compatibleNozzles.some((nozzle) => nozzle.id === values.nozzleId)) {
      setValue('nozzleId', '');
    }
    if (!values.nozzleId && compatibleNozzles.length === 1) {
      setValue('nozzleId', compatibleNozzles[0].id);
    }
  }, [compatibleNozzles, setValue, values.nozzleId]);
  ...
  if (!canRunPos)
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] opacity-40">
        ...
        <h2 className="text-xl font-black uppercase">{t('posPage.noPermission')}</h2>
        <p className="text-sm font-bold">{t('posPage.noPermission')}</p>
      </div>
    );
          ...
          <div className="bg-card border border-border rounded-[2rem] p-8 shadow-sm space-y-8">
            <div className="flex items-center justify-between border-b border-border pb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-black tracking-tight">{t('posPage.title')}</h2>
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">
                    Session: {user?.name} (Shift #4521)
                  </p>
                </div>
              </div>
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => reset()}
                  className="px-4 py-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:bg-muted rounded-xl transition-all"
                >
                  Clear Cart
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-6">
                ...
                <SelectField
                  ...
                  options={
                    products?.map((p) => ({
                      label: `${p.name} (${symbol}${p.pricePerUnit}/L)`,
                      value: p.id,
                    })) || []
                  }
                  required
                ...
                <SelectField
                  name="nozzleId"
                  label="Active Nozzle"
                  options={compatibleNozzles.map((n) => ({
                    label: `${n.pumpCode} - ${n.nozzleCode}`,
                    value: n.id,
                  }))}
                  hint="Required for dispenser audit and inventory tracking."
                  required
                />
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <NumberField
                    ...
                    hint={
                      !canOverridePrice ? 'Sales void permission required for price override.' : ''
                    }
                  />
                  {canOverridePrice &&
                    watch('pricePerUnit') !==
                      products?.find((p) => p.id === watch('productId'))?.pricePerUnit && (
                      <TextField
                        name="priceOverrideReason"
                        label="Override Rationale"
                        placeholder="e.g. Bulk discount agreement..."
                        required
                      />
                    )}
                </div>
            ...
            <div className="pt-8 border-t border-border">
                ...
                {(
                  ...
                  >
                    ...
                    <input
                      ...
                      className="w-full bg-transparent border-none p-0 text-lg font-black tabular-nums text-right focus:ring-0 outline-none"
                      placeholder="0.00"
                    />
                  </div>
                ))}
              ...
              {errors.payment && (
                <p className="text-[10px] font-bold text-rose-500 mt-3">{errors.payment.message}</p>
              )}
            </div>
          </div>
        </form>
        ...
        <aside className="lg:col-span-4 space-y-6">
          <div className="bg-slate-900 text-white rounded-[2rem] p-8 shadow-2xl space-y-12 h-full flex flex-col">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold">{fmt(values.pricePerUnit ?? 0)}</span>
                </div>
                {values.discount > 0 && (
                  <div className="flex justify-between text-sm text-emerald-400">
                    <span>Discount:</span>
                    <span className="font-bold">-{fmt(values.discount)}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col justify-center text-center py-12">
              ...
              <p className="text-6xl font-black tracking-tighter">{fmt(totalDue)}</p>
            </div>

            <div className="space-y-4">
              <p className="text-[9px] font-black uppercase text-white/30 text-center tracking-widest">
                Quick Payment Presets
              </p>
              <div className="grid grid-cols-3 gap-2">
                {QUICK_PAYMENT_PRESETS.map((amt) => (
                  <button
                    key={amt}
                    type="button"
                    onClick={() => {
                      const remaining =
                        totalDue -
                        (values.payment.card + values.payment.mobile + values.payment.voucher);
