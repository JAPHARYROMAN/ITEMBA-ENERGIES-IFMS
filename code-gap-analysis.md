...
- **0 unit test spec files** exist inside `apps/api/src/modules/` for any of the 16 business modules.
- Only 2 service spec files exist: `shifts.service.spec.ts` and `deliveries.service.spec.ts`, both located under their respective module directories.
- Jest is configured (`apps/api/jest.config.js`) but coverage is minimal.
...
16. Implement unit tests for all 16 API modules.
