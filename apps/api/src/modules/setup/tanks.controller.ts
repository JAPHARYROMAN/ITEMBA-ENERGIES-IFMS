...
import { TanksService, type TankItem } from './tanks.service';
...
export class TanksController extends BaseListController {
  ...
  @ApiOperation({ summary: 'List tanks' })
  @ApiResponse({ status: 200 })
  async list(@Query() query: ListQueryDto): Promise<ListResponse<TankItem>> {
    const params = getListParams(query);
    const { data, total } = await this.tanksService.findPage({
      page: query.page,
